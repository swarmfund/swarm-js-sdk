  ...
  _createClass(CallBuilder, {
    ...
    _getRequestConfig: {
      ...
      value: function _getRequestConfig(url, keypair) {
        ...
        var validUntil = Math.floor(new Date().getTime() / 1000 + constants.SIGNATURE_VALID_SEC).toString();
      ...
      value: function _toCollectionPage(json) {
        ...
        return {
          ...
          next: function (keypair) {
            if (keypair) {
              var config = _this._getRequestConfig(URI(json._links.next.href), keypair);
              return _this._sendNormalRequest(URI(json._links.next.href), config).then(function (r) {
          ...
          prev: function (keypair) {
            if (keypair) {
              var config = _this._getRequestConfig(URI(json._links.next.href), keypair);
              return _this._sendNormalRequest(URI(json._links.prev.href), config).then(function (r) {
                return _this._toCollectionPage(r);
              });
            } else {
